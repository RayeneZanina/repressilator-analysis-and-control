...
The controller will then try to keep the regime close to the Hopf bifurcation boundary, slightly above it so that the oscillations do not die out. For this, we use a PID controller. Note that this controller will be regulating the parameter $u$, and has access to information directly from the system that is not accessible in real conditions. It is possible to infer these parameters by collecting data, and we will be investigating this later.
