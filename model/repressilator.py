...
class Repressilator:
    ...
    def jacobian(self,P, u):
        beta = u * self.alpha * self.n * (P ** (self.n - 1)) / ((1 + P ** self.n) ** 2)
        beta1, beta2, beta3 = beta[0], beta[1], beta[2]
        return np.array([
            [-self.gamma, 0, -beta3],
            [-beta1, -self.gamma, 0],
            [0, -beta2, -self.gamma]
...
class Repressilator_one_control :
    ...
    def update(self, state, u):
        P1, P2, P3 = state
        dP1 = self.alpha / (1 + P3**self.n) - self.gamma * P1
        dP2 = self.alpha / (1 + P1**self.n) - self.gamma * P2
        dP3 = u * self.alpha / (1 + P2**self.n) - self.gamma * P3
    ...
    def beta(self, P):
        return self.alpha * self.n * (P ** (self.n - 1)) / ((1 + P ** self.n) ** 2)
    
    def regime(self, u):
        P_eq = self.get_equilibrium(u)
        return np.prod(self.beta(P_eq)) * u - 8 * self.gamma ** 3
